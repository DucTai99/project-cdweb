...
const FilterProductDisplay = (props) => {
  ...
  useEffect(() => {
    ...
    console.log(
      "[FilterProductDisplay] history.location.search ==== " +
  ...
  if (filterProductsReducer.isLoading) {
    ...
    if (filterProductsReducer.hasOwnProperty("data")) {
      ...
      if (filterProductsReducer.hasOwnProperty("statusCode")) {
        log.info(
          `[Home]: homeAPIData.statusCode = ${filterProductsReducer.statusCode}`
        );
  ...
  const handleImageClick = (selectedProduct) => () => {
    log.debug(
      `[FilterProductDisplay] dispatching the selected product = ${JSON.stringify(
        selectedProduct
      )}`
    );
    dispatch({
  ...
  const renderProductList = (productList) => {
    log.trace(
      `[FilterProductDisplay] Rendering renderImageList imageList = ${JSON.stringify(
        productList
      )}`
    );
    ...
    return productList.map((info) => {
      ...
      return (
        ...
        >
          <Grid item>
            <Link
              to={`${DETAILS_ROUTE}${history.location.search}::product_id=${info.id}`}
          ...
          <Grid item>
            <Link
              to={`${PRODUCTS_ROUTE}?q=brand=${info.productBrandCategory.id}`}
            >
              <div
          ...
          <Grid item style={{ fontSize: "16px", fontWeight: "bold" }}>
            {(info.price * 1300).toLocaleString("it-IT", {
              style: "currency",
              currency: "VND",
            })}
          </Grid>
