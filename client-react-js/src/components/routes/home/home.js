...
const Home = (props) => {
  ...
  if (homeAPIData.isLoading) {
    ...
    return (
      <Spinner
        textComponent={
          ...
          >
            <Grid item>
              Please wait! This will take 1-2 minutes to load page for the first
              time.
            </Grid>
            <Grid item>
              The backend service on heroku was in sleep mode and now its
              started.
            </Grid>
            <Grid item>
              It will sleep again if the backend service is inactive for 30
              minutes.
            </Grid>
          </Grid>
        }
    ...
    if (
      ...
      log.info(
        `[Home]: homeAPIData.data length didn't matched` +
...
export default connect(null, { getDataViaAPI, setDefaultSearchSuggestions })(
  Home
);
