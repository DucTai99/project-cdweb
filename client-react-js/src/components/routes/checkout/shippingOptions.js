...
const shippingOptionsData = {
  free: {
    ...
    transitLabel: "Transit time: 6-10 business days",
    estimatedDays: [6, 10],
  },
  premium: {
    price: `${(deliveryPrices.premium * 2300).toLocaleString("it-IT", {
      style: "currency",
      currency: "VND",
    })}`,
    label: "Premium",
  ...
  express: {
    price: `${(deliveryPrices.express * 2300).toLocaleString("it-IT", {
      style: "currency",
      currency: "VND",
    })}`,
    label: "Express",
    transitLabel: "Transit time: 1-3 business days",
    estimatedDays: [1, 3],
  },
};
...
export function ShippingOptions() {
  ...
  const [shippingOptionState, setShippingOptionState] = React.useState({
    value: "free",
    submitted: false,
  });
  ...
  const handleRadioBtnChange = (event) => {
    ...
    switch (event.target.value) {
      ...
      default:
        log.error(
          `[ShippingOptions] Unexpected ShippingOptions value = ${event.target.value}`
        );
        return null;
    }
    ...
    dispatch({
      ...
      payload: {
        estimatedDate: getEstimatedDeliveryDate(
          shippingOptionsData[selectedOption].estimatedDays
        ),
  ...
  const editBtnHandler = () => {
    setShippingOptionState({
      value: shippingOptionState.value,
      submitted: false,
    });
  };
  ...
  const renderSelectedOption = () => {
    return (
      <Grid item container xs={11}>
        {renderRadioBtnLabel(
          shippingOptionsData[shippingOptionState.value].label,
  ...
  const renderRadioBtnLabel = (lblText, helperText, price, estimatedDays) => {
    return (
      <Grid
        ...
        xs={12}
        sm={12}
        style={{ width: "inherit", paddingTop: 20 }}
      >
        <Grid item xs={7}>
  ...
  const renderImages = () => {
    if (
      !shoppingBagProducts.hasOwnProperty("data") ||
      Object.keys(shoppingBagProducts.data).length === 0
    ) {
    ...
    return (
      <FormControl component="fieldset" style={{ width: "inherit" }}>
        ...
        >
          <FormControlLabel
            value="free"
            control={<Radio style={{ marginLeft: 15 }} />}
            style={{ width: "inherit", margin: "inherit" }}
            classes={{ label: classes.formControlLabel }}
            label={renderRadioBtnLabel(
              shippingOptionsData.free.label,
          ...
          <FormControlLabel
            ...
            classes={{ label: classes.formControlLabel }}
            label={renderRadioBtnLabel(
              shippingOptionsData.premium.label,
          ...
          <FormControlLabel
            ...
            classes={{ label: classes.formControlLabel }}
            label={renderRadioBtnLabel(
              shippingOptionsData.express.label,
    ...
    <Grid item style={{ width: "100%", height: "fit-content" }}>
      ...
      <Grid item container sm={12} justify="center">
        {shippingOptionState.submitted
          ...
          : renderShippingRadioButtons()}
      </Grid>
      <Grid item container justify="center" sm={12} style={{ paddingTop: 20 }}>
