...
function PriceDetails() {
  ...
  return (
    <Grid container justify="center">
      ...
      {renderGridData(
        "Bag Total",
        `${(cartTotal * 1300).toLocaleString("it-IT", {
          style: "currency",
          currency: "VND",
        })}`,
        { ...paymentStyles, paddingTop: 10 }
      )}
      {renderGridData(
        "Shipping",
        `${(deliveryCharges * 2300).toLocaleString("it-IT", {
          style: "currency",
          currency: "VND",
        })}`,
        { ...paymentStyles, paddingTop: 10 }
      ...
      {renderGridData(
        "Order Total",
        `${((cartTotal + deliveryCharges) * 1300).toLocaleString("it-IT", {
          style: "currency",
          currency: "VND",
        })}`,
