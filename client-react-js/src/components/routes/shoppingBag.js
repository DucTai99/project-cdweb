...
function ShoppingBag(props) {
  ...
  const shoppingBagProducts = useSelector(
    (state) => state.shoppingBagProductReducer
  );
  const dispatch = useDispatch();
  const [itemRemovalModalState, setItemRemovalModalState] = useState({
    active: false,
    productId: null,
  });
  let cartTotal = 0;
  ...
  const breadcrumbLinks = [
    ...
    },
    {
      name: "Products",
      link: `${
        getStringBeforeLastDelimiter(history.location.pathname, "/details") +
  ...
  const getCartTotal = () => {
    log.info(
      `[ShoppingBag] ShoppingBag = ${JSON.stringify(shoppingBagProducts)}`
    );
    log.info(`[ShoppingBag] addToCart = ${JSON.stringify(addToCart)}`);
    if (
      shoppingBagProducts.data &&
      Object.keys(addToCart.productQty).length > 0
    ) {
    ...
    return (cartTotal * 1300).toLocaleString("it-IT", {
      style: "currency",
      currency: "VND",
    });
  };
  ...
  const removeConfirmBtnClickHandler = () => {
    ...
    if (itemRemovalModalState.productId) {
      let newAddToCart = addToCart;
      newAddToCart.totalQuantity -=
        newAddToCart.productQty[itemRemovalModalState.productId];
      newAddToCart.productQty = _.omit(
        newAddToCart.productQty,
        itemRemovalModalState.productId
      );
      Cookies.set(SHOPPERS_PRODUCT_INFO_COOKIE, newAddToCart, { expires: 7 });
      dispatch({
        type: ADD_TO_CART,
  ...
  if (shoppingBagProducts.isLoading) {
    ...
    if (shoppingBagProducts.hasOwnProperty("data")) {
      ...
      if (shoppingBagProducts.hasOwnProperty("statusCode")) {
        log.info(
          `[ShoppingBag]: ShoppingBag.statusCode = ${shoppingBagProducts.statusCode}`
        );
    ...
    for (const [id, product] of Object.entries(shoppingBagProducts.data)) {
      shoppingBagProductsList.push(
        <Grid
          ...
          container
          key={product.id}
          style={{ border: "1px solid #eaeaec", margin: "1rem 0" }}
        >
          ...
          <Grid item container xs={7} sm={9}>
            <Grid item container direction="column" sm={6} spacing={1}>
              ...
              <Grid item>
                <DropdownSection
                  ...
                  selectedValue={
                    addToCart.productQty.hasOwnProperty(id)
                      ? {
                          id: id,
            ...
            >
              {`Qty: ${addToCart.productQty[product.id]} x ${(
                product.price * 1300
                ...
                `${(
                  addToCart.productQty[product.id] *
                  product.price *
                  1300
                ).toLocaleString("it-IT", {
                  style: "currency",
          ...
          <Grid
            ...
            container
            justify="flex-end"
            style={{ padding: "0.5rem 0.7rem 0.5rem 0" }}
          >
            <Grid item sm={3}>
              <Button
                variant="contained"
    ...
    <>
      ...
      <Grid container justify="center" style={{ height: "100%" }}>
        ...
        <Grid item sm={8} md={3}>
          <Paper
            square
            style={{ width: "inherit", position: "sticky", top: 80 }}
          >
            <PriceDetails
              buttonName="Proceed To Checkout"
              onBtnClickHandler={continueBtnClickHandler}
            />
