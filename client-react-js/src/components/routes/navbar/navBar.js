...
const NavBar = (props) => {
  ...
  const { isSignedIn, tokenId, firstName } = useSelector(
  ...
  const setAddToCartValuesFromCookie = () => {
    ...
    if (savedProductsFromCookie) {
      ...
      for (const [, qty] of Object.entries(
        savedProductsFromCookie.productQty
      )) {
      ...
      log.info(
        `[BagButton] savedProductsFromCookie = ${JSON.stringify(
          savedProductsFromCookie
        )}`
      );
              ...
              dispatch({
                ...
                payload: {
                  firstName: auth.currentUser.get().getBasicProfile()
                    ? auth.currentUser.get().getBasicProfile().getGivenName()
                    : null,
                  oAuth: auth,
                },
              });
  ...
  if (tabsAPIData.isLoading) {
    ...
    if (tabsAPIData.hasOwnProperty("data")) {
      if (Object.entries(tabsAPIData.data).length !== TABS_API_OBJECT_LEN) {
        log.info(
          `[NavBar]: tabsAPIData.data length didn't matched` +
      ...
      if (tabsAPIData.hasOwnProperty("statusCode")) {
        log.info(
          `[NavBar]: tabsAPIData.statusCode = ${tabsAPIData.statusCode}`
        );
        props.errorHandler();
  ...
  const changeAuthStatusHandler = () => {
    log.info(
      `[Navbar] handleSignOutClick isSignedIn = ${googleAuthReducer.isSignedInUsingOAuth}`
    );
    ...
    if (mobileSearchState) {
      return (
        <SearchBar
          size="medium"
          device="mobile"
          handleClose={handleMobileSearchClose}
        />
      );
    }
    return (
      <>
        <Grid item>
          <IconButton onClick={handleMobileSearchOpen} edge="end">
          ...
          <Toolbar classes={{ root: classes.toolBarRoot }}>
            <Grid container alignItems="center">
              ...
              {!mobileSearchState ? (
                <Grid item>
                  <Link to="/">
                    <Typography className={classes.title}>Shoppers</Typography>
                  </Link>
                </Grid>
              ) : null}
              ...
              <Hidden xsDown>
                ...
                {renderIndependentElem(
                  changeAuthStatusHandler,
                  authIcon,
                  authLabel,
                  2
                )}
                ...
                {renderIndependentElem(
                  changePageToShoppingBagHandler,
                  <BagButton />,
                  "Bag",
                  0
                )}
