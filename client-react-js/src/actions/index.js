...
export const signIn = (formValues) => async (dispatch) => {
  ...
  const response = await authServiceAPI.post("/authenticate").catch((err) => {
    log.info(
      `[ACTION]: dispatch HANDLE_SIGN_IN_ERROR err.message = ${err.message}`
    );
  ...
  if (response) {
    if (response.data.jwt) {
      ...
      log.info(
        `[ACTION]: dispatch HANDLE_SIGN_IN_ERROR response.data.error = ${response.data.error}`
      );
...
export const signOutUsingOAuth = (googleAuth) => async (dispatch) => {
  log.info(
    `[signOutUsingOAuth] googleAuth = ${googleAuth}, ` +
...
export const signUp = (formValues) => async (dispatch) => {
  ...
  const response = await authServiceAPI
    ...
    .catch((err) => {
      log.info(
        `[ACTION]: signUp dispatch HANDLE_SIGN_UP_ERROR err.message = ${err.message}.`
      );
  ...
  if (response) {
    if (response.data.account_creation_status === "success") {
      log.info(
        `[ACTION]: dispatch HANDLE_SIGN_UP account_creation_status = ${response.data.account_creation_status}.`
      );
      history.push("/signin");
    } else {
      console.log("response.data.error_msg = " + response.data.error_msg);
      log.info(
        `[ACTION]: dispatch HANDLE_SIGN_UP_ERROR response.data.error_msg = ${response.data.error_msg}.`
      );
      dispatch({
...
export const getDataViaAPI =
  (type, route, query, synchronous = true) =>
  async (dispatch) => {
    if (route) {
      if (query) {
        route += query;
...
export const loadFilterAttributes = (filterQuery) => (dispatch) => {
  log.info(
    `[ACTION]: loadFilterAttributes Calling Filter API filterQuery = ${filterQuery}`
  );
...
export const setDefaultSearchSuggestions = () => (dispatch) => {
  ...
  searchSuggestionServiceAPI
    ...
    .catch((err) => {
      log.info(
        `[ACTION]: unable to fetch response for API = ${DEFAULT_SEARCH_SUGGESTION_API}`
      );
      dispatch({ type: SEARCH_KEYWORD_ERROR });
    });
};
