...
export const googleAuthReducer = (
  state = { isSignedInUsingOAuth: null, firstName: null, oAuth: null },
  action
) => {
  switch (action.type) {
...
export const signUpReducer = (
  state = { errorMsg: null, timestamp: null },
  action
) => {
  // timestamp is used to update the state so that
  // we can stop loading progress component
...
export const homePageDataReducer = (state = { isLoading: true }, action) => {
  ...
  switch (action.type) {
    case LOAD_HOME_PAGE:
      log.trace(
        `[HOME_SCREEN_REDUCER]: action.payload = ${JSON.stringify(
          action.payload
        )}`
      );
...
export const addToCartReducer = (
  state = { totalQuantity: 0, productQty: {} },
  action
) => {
  switch (action.type) {
    case ADD_TO_CART:
...
export const shoppingBagProductReducer = (
  state = { isLoading: true },
  action
) => {
  switch (action.type) {
...
export const shippingAddressReducer = (
  state = INITIAL_SHIPPING_ADDRESS_STATE,
  action
) => {
  switch (action.type) {
...
export const shippingOptionReducer = (
  state = INITIAL_SHIPPING_OPTION_STATE,
  action
) => {
  switch (action.type) {
...
export const paymentResponseReducer = (
  state = { error: false, errorMsg: null, timestamp: null },
  action
) => {
  switch (action.type) {
...
export const searchKeywordReducer = (
  state = { error: false, errorMsg: null, data: [{ keyword: " " }] },
  action
) => {
  switch (action.type) {
    case SEARCH_KEYWORD:
